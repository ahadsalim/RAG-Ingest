...
def log(msg):
    ...
    log_file = os.path.join(state["script_dir"], "legal_image_processor.log")
    try:
...
def get_jpg_images():
    """Get all JPG images in the script directory."""
    jpg_files = []
    for file in os.listdir(state["script_dir"]):
...
def call_gpt_vision(image_files):
    ...
    try:
        ...
        for img_file in image_files:
            img_path = os.path.join(state["script_dir"], img_file)
...
if __name__ == '__main__':
    log(f"Legal Image Processor v{VERSION} starting...")
